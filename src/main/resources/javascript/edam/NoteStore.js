...
NoteStore_expungeNotes_args.prototype.write = function(output) {
  ...
  if (this.noteGuids) {
    ...
    for (var iter277 in this.noteGuids)
    {
      if (this.noteGuids.hasOwnProperty(iter277))
      {
        iter277 = this.noteGuids[iter277];
...
NoteStore_sendMessageToSharedNotebookMembers_args.prototype.write = function(output) {
  ...
  if (this.recipients) {
    ...
    for (var iter301 in this.recipients)
    {
      if (this.recipients.hasOwnProperty(iter301))
      {
        iter301 = this.recipients[iter301];
  ...
  while (true)
  {
    ...
    switch (fid)
    {
      ...
      if (ftype == Thrift.Type.LIST) {
        ...
        {
          var elem316 = null;
          elem316 = input.readI64().value;
          this.sharedNotebookIds.push(elem316);
        }
        input.readListEnd();
      } else {
        input.skip(ftype);
...
NoteStore_expungeSharedNotebooks_args.prototype.write = function(output) {
  ...
  if (this.sharedNotebookIds) {
    ...
    for (var iter317 in this.sharedNotebookIds)
    {
      if (this.sharedNotebookIds.hasOwnProperty(iter317))
      {
        iter317 = this.sharedNotebookIds[iter317];
