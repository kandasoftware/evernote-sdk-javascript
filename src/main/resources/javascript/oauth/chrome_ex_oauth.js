...
ChromeExOAuth.prototype.authorize = function(callback) {
  if (this.hasToken()) {
    ...
    window.chromeExOAuthOnAuthorize = function(params) {
      callback(params);
    };
...
ChromeExOAuth.initCallbackPage = function() {
  ...
  oauth.initOAuthFlow(function (params) {
    background_page.chromeExOAuthOnAuthorize(params);
...
/**
 * Stores an parameter for the configured scope.
 * @param {String} paramName The parameter name.
 * @param {String} value The parameter value.
 */
ChromeExOAuth.prototype.setParameter = function(paramName, value) {
  localStorage[paramName + encodeURI(this.oauth_scope)] = value;
};

/**
 * Retrieves any stored parameter for the configured scope.
 * @param {String} paramName The parameter name.
 */
ChromeExOAuth.prototype.getParameter = function(paramName) {
  return localStorage[paramName + encodeURI(this.oauth_scope)];
};
...
ChromeExOAuth.prototype.onAccessToken = function(callback, xhr) {
  if (xhr.readyState == 4) {
    ...
    if (xhr.status == 200) {
      ...
      this.setParameter("edam_expires", params["edam_expires"]);
      this.setParameter("edam_noteStoreUrl", params["edam_noteStoreUrl"]);
      this.setParameter("edam_shard", params["edam_shard"]);
      this.setParameter("edam_webApiUrlPrefix", params["edam_webApiUrlPrefix"]);
      this.setToken(token);
